...
bot = telebot.TeleBot('1194122886:AAFbl9xTBJreivZPHQ7lASGYwVLYZTaNQ4U')
...
anypay = types.KeyboardButton('AnyPay')
...
def next3(message):
    ...
    if message.text == 'Назад':
        ...
        if int(message.text) < 5:
            ...
            msg = bot.send_message(message.chat.id, "Хорошо,выбери режим скорости:",reply_markup=markupSPEED)
        ...
        if int(a[1]) < l:
            ...
            if len(data) == 6:
                ...
                if r.startswith('{"error":'):
                    bot.send_message(message.chat.id, 'Задание создать не удалось, возможно проблема в ссылке',reply_markup=markup)
                else:
                    conn = sqlite3.connect('example.db')
                    c = conn.cursor()
                    t = int(message.chat.id)
                    ...
                    ofaa = ofaa + 1
                    data2.append(ofaa)
                    bot.send_message(message.chat.id, 'Заказ успешно принят. После его выполнения я напишу',reply_markup=markup)
...
def balance2(message):
    if message.text == 'Назад':
        ...
        msg = bot.send_message(message.chat.id, "Через что пополнить?", reply_markup=markupchto)
...
def chtos(message):
    if message.text == 'Назад':
        bot.send_message(message.chat.id, 'Добро пожаловать в главное меню! Выберите действие:',reply_markup=markup)
    elif message.text  == 'AnyPay':
        msg2 = bot.send_message(message.chat.id, "На сколько рублей хотите пополнить?(мин-5)", reply_markup=markupnazad)
        bot.register_next_step_handler(msg2, anypays)
    elif message.text  == 'Qiwi':
        msg4 = bot.send_message(message.chat.id, "На сколько рублей хотите пополнить?(мин-5)", reply_markup=markupnazad)
...
def anypays(message):
    ...
    if message.text == 'Назад':
        ...
        currency = 'RUB'
        pay_id = ax
        secret_key = 'pLoG7i8WpT0aBahXdcZoHpGHCs'
        arr_sign = bytes(f'{currency}:{amount}:{secret_key}:{project_id}:{pay_id}', encoding='utf-8')
        result = hashlib.md5(arr_sign)
        a = result.hexdigest()
        a = str(a)
        params = {'merchant_id': project_id, 'amount': amount,'desc':'Оплатите для пополнения баланса!','currency': currency, 'pay_id': pay_id, 'sign': a}
        ...
        msg = bot.send_message(message.chat.id, "На сколько рублей хотите пополнить?(мин-5)", reply_markup=markupnazad)
    ...
    if message.text == 'Назад':
        ...
        try:
            ...
            if a['result']['payments'] == None:
                ...
                if a['result']['payments']["pay_id"] == int(pay_id):
                    if a['result']['payments']["status"] == "paid":
                        a = int(yes)
                        t = (int(message.chat.id),)
                        c.execute("""DELETE from payments where id = ?""", t)
                        t = (int(message.chat.id),)
...
def balance4(message):
    if message.text == 'Назад':
        ...
        try:
            ...
            a = c.fetchone()
            print(req['data'])
            print(req['data'][3]['comment'])
            print(req['data'][3]['sum']['amount'])
            if len(req['data']) == 0:
    ...
    if message.chat.id == group_id:
        if str(message.text).startswith('Бан') or str(message.text).startswith('бан') or str(message.text).startswith('ban') or str(message.text).startswith('Ban'):
            ...
            try:
                ...
                bot.send_message(group_id, 'Что-то пошло не так!\nВозможно вы ответили не на сообщение репорта.')
    else:
            ...
            if a == []:
                ...
                for i in range((len(a))):
                    for b in e:
                        if a[i]['network'] == b[1]:
                            if a[i]['section'] == b[2]:
                                if a[i]['ordered_count'] == b[4]:
                                    if a[i]['status'] == 'success':
                                        bot.send_message(b[0], 'Заказ успешно выполнен!')
                                        t = (int(b[6]),)
